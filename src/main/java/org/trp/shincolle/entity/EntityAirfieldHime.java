...
public class EntityAirfieldHime extends EntityShipBase {
    ...
    private void updateServerLogic() {
        ...
        for (LivingEntity target : targets) {
            ...
            if (
                target == this ||
                target.getHealth() / target.getMaxHealth() >= 0.96f
            ) {
                continue;
            }
    ...
    public org.trp.shincolle.entity.base.EntityMountBase summonMountEntity() {
        return new EntityMountAfH(
            org.trp.shincolle.init.ModEntities.MOUNT_AF_H.get(),
            this.level()
        );
    }
}
